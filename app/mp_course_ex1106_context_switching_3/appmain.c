...
/*
    Enhancement:
        * os_start via system call using SVC
*/
...
void os_start(void)
{
    ...
    svc_exc_return = 0xFFFFFFFDUL; // EXC_RETURN value for SVC handler (return to Thread mode with PSP)
    
    __set_PSP((PSP_array[curr_task] + 8*4)); // Set PSP to top of task 0 stack (When the first task is performed, the context (R4 - R11) is simply discarded without recovery.)
